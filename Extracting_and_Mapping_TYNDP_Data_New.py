...
Goal
----
Create node-level TYNDP Generation_Data.csv and Demand_Data.csv WITHOUT any
powerplant-matching allocation.

Updated node concept
--------------------
Nodes are derived from a combined grid-elements CSV that contains bus_u / bus_v.
- Non-AT: one node per country code (e.g. "DE", "CZ", ...)
- AT: Austrian substations as nodes (e.g. "BISAMBERG", "BUERS", ...),
      including AT-side buses of tielines (to preserve correct interconnectors).

IMPORTANT UPDATE (this revision)
--------------------------------
If there are NO AT substations in Lines_AT_and_Tielines_Parameters.csv (i.e. only
country codes as buses), then Austria is handled at COUNTRY LEVEL:
- node set contains only "AT" for Austria
- NO GSK splitting is applied (generation/storage remain on "AT")
- demand allocation remains country-level automatically

Allocation
----------
Generation:
- Non-AT: uniform across that country's nodes (usually 1 node = country code)
- AT: via Generation Shift Key (GSK) by Bundesland + group (optional, only if AT substations exist)

Storage:
- Non-AT: uniform
- AT: GSK optional (only if AT substations exist)

Demand:
- uniform across nodes of each country (including AT substations if they exist)

Expected in Substations_AT.py
-----------------------------
- SUBSTATION_LOOKUP[substation]["state"] = Bundesland for each AT substation
- GSK_STATE_TECH_SHARES[state][group] = share (sums to 1 across states per group)
"""
