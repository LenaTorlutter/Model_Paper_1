...
from typing import Iterable, List, Literal, Optional, Tuple, Union, Dict
...
class Outputs:
    ...
    # Weekly dual-price dumps for Stage 1
    ...
    full_stage0_results: str = "full_model_results_stage0_test.csv"
    full_stage0_prices: str = "full_stage0_prices_test.csv"
    full_stage0_pv: str = "full_stage0_pv_quantities_test.csv"

    full_follower_base: str = "full_follower_values_base_test.csv"
    full_stage1_base: str = "full_model_results_stage1_base_test.csv"

    # Full concatenations of Stage 1 dual prices
    full_stage1_base_dual_prices: str = "full_stage1_base_dual_prices_test.csv"
...
def concat_weekly_csvs(results_dir: str, out_path: str) -> pd.DataFrame:
    files = sorted([f for f in os.listdir(results_dir) if f.lower().endswith("_test.csv")]) if os.path.isdir(results_dir) else []
    if not files:
...
def instrument_full_paths(paths: Paths, instrument: str) -> Tuple[str, str]:
    slug = instrument_slug(instrument)
    full_follower = os.path.join(paths.base_dir, f"full_follower_values_instrument__{slug}_test.csv")
    full_system = os.path.join(paths.base_dir, f"full_model_results_stage1_instrument__{slug}_test.csv")
...
def instrument_dual_price_full_path(paths: Paths, instrument: str) -> str:
    slug = instrument_slug(instrument)
    return os.path.join(paths.base_dir, f"full_stage1_dual_prices_instrument__{slug}_test.csv")
...
def dump_system_window(model, *, window_id: int, results_dir: str, label: str) -> None:
    ...
    out = os.path.join(results_dir, f"window_{window_id:03d}_system_{label}_test.csv")
...
def dump_follower_window(df: pd.DataFrame, *, window_id: int, results_dir: str, label: str) -> None:
    if df is None or df.empty:
        ...
        return
    out = os.path.join(results_dir, f"window_{window_id:03d}_follower_{label}_test.csv")
    df.to_csv(out, index=False)
...
# =============================================================================
# Stage 1 base capacity tariff construction
# =============================================================================

def compute_node_capacity_tariff_by_billing_block_from_stage1_base(
    follower_base_dfs: List[pd.DataFrame],
    *,
    billing_weeks: int,
    mode: Literal["per_node", "uniform"] = "uniform",
) -> Tuple[pd.DataFrame, Dict[int, Dict[str, float]]]:
    """
    Compute capacity-tariff rates from Stage-1 BASE follower outputs.

    Modes
    -----
    per_node:
        cap_tariff_{b,n} = sum_{t in b} fees(n,t) / max_{t in b} imports_power(n,t)

    uniform:
        cap_tariff_{b} = sum_{n,t in b} fees(n,t) / max_{n,t in b} imports_power(n,t)
        and then assign cap_tariff_{b,n} = cap_tariff_{b} for all nodes.

    Requires follower BASE export columns:
      - node
      - start_week
      - imports_power   (MW)
      - fees            (EUR over timestep)
    """
    if not follower_base_dfs:
        raise ValueError("Stage 1 BASE follower outputs are empty (no dfs).")

    df = pd.concat([d for d in follower_base_dfs if d is not None and not d.empty], ignore_index=True)
    if df.empty:
        raise ValueError("Stage 1 BASE follower outputs are empty after concat.")

    needed = {"node", "start_week", "imports_power", "fees"}
    missing = needed - set(df.columns)
    if missing:
        raise ValueError(f"Stage 1 BASE follower results missing columns: {sorted(missing)}")

    if billing_weeks <= 0:
        raise ValueError("billing_weeks must be a positive integer.")
    ...
    # billing block based on week number (weeks start at 1)
    df["billing_block"] = ((df["start_week"] - 1) // int(billing_weeks)).astype(int)

    nodes_all = sorted(df["node"].unique().tolist())

    if mode == "per_node":
        out = (
            df.groupby(["billing_block", "node"], as_index=False)
              .agg(
                  block_start_week=("start_week", "min"),
                  block_end_week=("start_week", "max"),
                  total_grid_fees_eur=("fees", "sum"),
                  max_import_mw=("imports_power", "max"),
              )
              .sort_values(["billing_block", "node"])
              .reset_index(drop=True)
        )

        out["cap_tariff_eur_per_mw"] = np.where(
            out["max_import_mw"] > 0,
            out["total_grid_fees_eur"] / out["max_import_mw"],
            0.0,
        )

    elif mode == "uniform":
        # 1) node-wise peaks inside each billing block
        node_peaks = (
            df.groupby(["billing_block", "node"], as_index=False)
              .agg(
                  node_peak_import_mw=("imports_power", "max"),
              )
        )
    
        # 2) block totals: fees summed over all rows, denominator is sum of node peaks
        blk_fees = (
            df.groupby(["billing_block"], as_index=False)
              .agg(
                  block_start_week=("start_week", "min"),
                  block_end_week=("start_week", "max"),
                  total_grid_fees_eur=("fees", "sum"),
        ...
        blk_den = (
            node_peaks.groupby(["billing_block"], as_index=False)
                     .agg(sum_of_node_peak_import_mw=("node_peak_import_mw", "sum"))
        )
    
        blk = blk_fees.merge(blk_den, on="billing_block", how="left").fillna({"sum_of_node_peak_import_mw": 0.0})
    
        blk["cap_tariff_eur_per_mw"] = np.where(
            blk["sum_of_node_peak_import_mw"] > 0,
            blk["total_grid_fees_eur"] / blk["sum_of_node_peak_import_mw"],
            0.0,
        )
    
        # expand to node-level table (uniform across nodes)
        out = blk.merge(pd.DataFrame({"node": nodes_all}), how="cross")
        out = out[[
            "billing_block", "node",
            "block_start_week", "block_end_week",
            "total_grid_fees_eur", "sum_of_node_peak_import_mw",
            "cap_tariff_eur_per_mw"
        ]].sort_values(["billing_block", "node"]).reset_index(drop=True)

    else:
        raise ValueError("mode must be one of: 'per_node', 'uniform'")

    # Build nested dict: block -> {node -> tariff}
    cap_tariff_by_block_node: Dict[int, Dict[str, float]] = {}
    for b, g in out.groupby("billing_block"):
        cap_tariff_by_block_node[int(b)] = dict(
            zip(g["node"].astype(str), g["cap_tariff_eur_per_mw"].astype(float))
        )

    return out, cap_tariff_by_block_node
...
def run_stage1_base(
    ...
    *,
    ...
    for w in range(horizon.num_windows):
        ...
        out_duals = os.path.join(weekly_stage1_base_dual_prices_dir, f"window_{w:03d}_stage1_base_dual_prices_test.csv")
...
def run_stage1_instrument(
    ...
    *,
    ...
    peak_rate_eur_per_mw: Union[float, Dict[int, Dict[str, float]]],
    billing_weeks: int,
) -> List[pd.DataFrame]:
    """
    ...
    for w in range(horizon.num_windows):
        ...
        # -------------------------
        # Endogenous capacity tariff (node-specific, per billing block)
        # -------------------------
        billing_block = int((int(start_week) - 1) // int(billing_weeks)) if billing_weeks > 0 else 0
        
        if isinstance(peak_rate_eur_per_mw, dict):
            # expected: {billing_block: {node: rate}}
            peak_rate_by_node: Dict[str, float] = peak_rate_eur_per_mw.get(billing_block, {})
        else:
            # scalar fallback (uniform)
            peak_rate_by_node = float(peak_rate_eur_per_mw)
        ...
        m_f_instr, df_f_instr = build_follower_model(
            ...
            peak_rate_eur_per_mw=peak_rate_by_node if instrument == "Capacity-Tariff" else 0.0,
        ...
        if df_f_instr is not None and not df_f_instr.empty:
            df_f_instr["billing_block"] = int(billing_block)
        
            if instrument == "Capacity-Tariff":
                if isinstance(peak_rate_by_node, dict):
                    df_f_instr["capacity_tariff_eur_per_mw_applied"] = (
                        df_f_instr["node"].astype(str).map(peak_rate_by_node).fillna(0.0)
                    )
                else:
                    df_f_instr["capacity_tariff_eur_per_mw_applied"] = float(peak_rate_by_node)
            else:
                df_f_instr["capacity_tariff_eur_per_mw_applied"] = 0.0
        ...
        out_duals = os.path.join(
            ...
            f"window_{w:03d}_stage1_{instrument_slug(instrument)}_dual_prices_test.csv",
        )
        df_duals.to_csv(out_duals, index=False)
...
def main() -> None:
    ...
    # --- Stage 1 BASE price from Stage 0 (PV-weighted)
    prices_by_t = (
        prices_df.groupby("t", as_index=False)["price"].mean()
        if not prices_df.empty
        else pd.DataFrame(columns=["t", "price"])
    )
    pv_by_t = (
        pv_df.groupby("t", as_index=False)["pv_generation"].sum()
        if not pv_df.empty
        else pd.DataFrame(columns=["t", "pv_generation"])
    )

    stage1_base_price = pv_weighted_price_over_horizon(prices_by_t, pv_by_t)
    ...
    follower_base_dfs = run_stage1_base(
        ...
        follower_control_area=CFG.follower_control_area,
        fees=CFG.fees,
        stage1_base_price=stage1_base_price,
        ...
        weekly_stage1_base_dual_prices_dir=paths.weekly_stage1_base_dual_prices_dir,
    )
    ...
    # Full concatenation of Stage-1 BASE dual prices
    concat_weekly_csvs(paths.weekly_stage1_base_dual_prices_dir, paths.full_stage1_base_dual_prices)
    
    # Node-specific endogenous capacity tariff per 4-week billing block (from Stage 1 BASE)
    tariff_node_table, cap_tariff_by_block_node = compute_node_capacity_tariff_by_billing_block_from_stage1_base(
        follower_base_dfs,
        billing_weeks=int(CFG.billing_weeks),
        mode="uniform", # or "per node"
    )

    tariff_node_path = os.path.join(paths.base_dir, "endogenous_capacity_tariff_by_block_node_test.csv")
    tariff_node_table.to_csv(tariff_node_path, index=False)
    print(f"\n✅ Saved node-specific endogenous capacity tariffs to: {tariff_node_path}\n")
    ...
    for instrument in INSTRUMENTS:
        ...
        # NEW: dual-price dir for this instrument
        dual_dir = instrument_dual_price_weekly_dir(paths, instrument)
        clean_csv_dir(dual_dir)
        ...
        follower_instr_dfs = run_stage1_instrument(
            ...
            weekly_stage1_instr_dual_prices_dir=dual_dir,
            peak_rate_eur_per_mw=cap_tariff_by_block_node,  # dict: billing_block -> tariff,
            billing_weeks=int(CFG.billing_weeks),
        )
        ...
        # NEW: full concatenation of Stage-1 INSTRUMENT dual prices
        full_dual_path = instrument_dual_price_full_path(paths, instrument)
        concat_weekly_csvs(dual_dir, full_dual_path)
