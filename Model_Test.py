# =============================================================================
# MODEL DEFINITIONS (SYSTEM + FOLLOWER) FOR ROLLING-HORIZON PIPELINE
# =============================================================================
# This script contains the mathematical optimization models used by the
# rolling-horizon pipeline:
#
#   - build_follower_model():
#       A nodal prosumer/follower model with PV, battery, and demand.
#       It maximizes the follower objective under different pricing
#       instruments:
#           * BASE
...
#
#       Features:
#           - PV generation split (self-consumption, battery charging, export, curtailment)
#           - Battery intertemporal storage dynamics
#           - Instrument-dependent export/import limits
#           - Optional node-specific capacity-based tariff (rolling peak logic)
#           - Detailed per-node export dataframe for system coupling
#
#   - create_model():
#       A full multi-node electricity system model (thermal, RES, PHS/HS,
#       DSR, batteries, network with PTDF formulation, NSE).
#       It minimizes total system cost subject to:
#           - Unit commitment & ramping
#           - Storage dynamics
#           - Renewable availability
#           - Network flow constraints
#           - Nodal demand equilibrium
#
#       The model can incorporate follower flows to represent prosumer
#       behavior in Stage 1.
#
#   - solve_model():
#       Thin solver wrapper for system model execution.
#
# The rolling-horizon orchestration, price construction, and result
# archiving are implemented separately in the pipeline script.
# =============================================================================
...
def build_follower_model(
    ...
    *,
    ...
    peak_rate_eur_per_mw: Union[float, Dict[str, float]],
    ...
    # -------------------------------------------------------------------------
    # Capacity tariff helper
    # -------------------------------------------------------------------------
    
    def normalize_peak_rate_by_node(peak_rate_eur_per_mw, nodes_in_ca):
        if isinstance(peak_rate_eur_per_mw, dict):
            rate_by_node = {str(n): float(peak_rate_eur_per_mw.get(str(n), 0.0)) for n in nodes_in_ca}
        else:
            rate_by_node = {str(n): float(peak_rate_eur_per_mw) for n in nodes_in_ca}
    
        bad = [(n, v) for n, v in rate_by_node.items() if (not np.isfinite(v)) or v < 0.0]
        if bad:
            raise ValueError(f"peak_rate_eur_per_mw must be finite and >= 0. Bad examples: {bad[:5]}")
        return rate_by_node
    
    peak_rate_by_node = normalize_peak_rate_by_node(peak_rate_eur_per_mw, nodes_in_ca)
    
    use_capacity_tariff = (instrument == "Capacity-Tariff") and (max(peak_rate_by_node.values()) > 0.0)
    ...
    m.peak_rate_eur_per_mw = Param(
        m.nodes,
        initialize=lambda _m, n: float(peak_rate_by_node.get(str(n), 0.0)),
        mutable=False,
    )
    ...
    def objective_rule(_m):
        ...
        apply_static_fees = (instrument != "Capacity-Tariff")  # <- or (not use_capacity_tariff)
        
        fees = 0.0
        if apply_static_fees:
            fees = (
        ...
        if use_capacity_tariff:
            power_charge = sum(
                _m.peak_rate_eur_per_mw[n] * (_m.peak_imports_new[n] + _m.peak_exports_new[n])
                for n in _m.nodes
            )
    ...
    for t in range(1, T + 1):
        for n in nodes_in_ca:
            ...
            if use_capacity_tariff:
                ...
                peak_rate_val_n = float(value(m.peak_rate_eur_per_mw[n]))
                power_charge_n = peak_rate_val_n * (peak_imports_new_n + peak_exports_new_n)
            else:
            ...
            rows.append({
                ...
                "peak_rate_eur_per_mw": peak_rate_val_n if use_capacity_tariff else 0.0,
...
def create_model(
    ...
    *,
    ...
    T_full = int(hours_per_week * weeks_per_step)
    ...
    if horizon_hours is None:
        ...
        if T > T_full:
            raise ValueError(f"horizon_hours={T} exceeds window length T_full={T_full}")

    # IMPORTANT: start hour stays the window start (your requirement)
    time_offset = int((start_week - 1) * hours_per_week)
    t0 = 0
    ...
    # penalty weight (tune)
    model.PEN_PHS_PROFILE = Param(initialize=1e6, mutable=True)
    
    # violations of the storage *band* (in whatever units phs_storage uses, likely MWh)
    model.phs_profile_under = Var(model.control_areas, model.timesteps, domain=NonNegativeReals)  # too low
    model.phs_profile_over = Var(model.control_areas, model.timesteps, domain=NonNegativeReals)  # too high

    def phs_profile_lb_soft(m, ca, t):
        ...
        lhs = sum(m.phs_storage[p, t] for p in m.phs_power_plants_by_control_area[ca])
        return lhs + m.phs_profile_under[ca, t] >= total_cap * target * 0.9

    def phs_profile_ub_soft(m, ca, t):
        if t == t0 or ca not in m.phs_power_plant_control_areas:
            return Constraint.Skip
        total_cap = sum(
            as_float(phs_df.loc[p, "Energy (max) [GWh]"]) * 1000.0
        ...
        target = float(phs_storage_profile.loc[t + time_offset, ca])
        lhs = sum(m.phs_storage[p, t] for p in m.phs_power_plants_by_control_area[ca])
        return lhs - m.phs_profile_over[ca, t] <= total_cap * target * 1.1
    
    model.phs_weekly_storage_profile_constraint_1 = Constraint(
        model.control_areas, model.timesteps, rule=phs_profile_lb_soft
    )
    model.phs_weekly_storage_profile_constraint_2 = Constraint(
        model.control_areas, model.timesteps, rule=phs_profile_ub_soft
    )
    ...
    # penalties (choose big numbers; see notes below)
    model.PEN_PTDFFLOW = Param(initialize=1e6, mutable=True)
    
    # violation slacks
    model.ptdf_violation_pos = Var(model.connections, model.timesteps, domain=NonNegativeReals)
    model.ptdf_violation_neg = Var(model.connections, model.timesteps, domain=NonNegativeReals)
    
    def ptdf_flow_soft_rule(m, c, t):
        if t == t0:
            return Constraint.Skip
        return (
            m.flow[c, t]
            - sum(m.PTDF[c, n] * m.exchange[n, t] for n in m.non_slack_nodes)
            == m.ptdf_violation_pos[c, t] - m.ptdf_violation_neg[c, t]
        )
    
    model.ptdf_flow_constraint = Constraint(model.connections, model.timesteps, rule=ptdf_flow_soft_rule)

    model.PEN_FLOWCAP = Param(initialize=1e6, mutable=True)
    
    model.flow_over_pos = Var(model.connections, model.timesteps, domain=NonNegativeReals)  # above upper bound
    model.flow_over_neg = Var(model.connections, model.timesteps, domain=NonNegativeReals)  # below lower bound (more negative)

    def flow_upper_soft_rule(m, c, t):
        if t == t0:
            return Constraint.Skip
        return m.flow[c, t] <= m.capacity_pos[c] * dt + m.flow_over_pos[c, t]
    
    def flow_lower_soft_rule(m, c, t):
        if t == t0:
            return Constraint.Skip
        return m.flow[c, t] >= -m.capacity_neg[c] * dt - m.flow_over_neg[c, t]
    
    model.flow_upper_bound = Constraint(model.connections, model.timesteps, rule=flow_upper_soft_rule)
    model.flow_lower_bound = Constraint(model.connections, model.timesteps, rule=flow_lower_soft_rule)
    
    model.PEN_EXCHBAL = Param(initialize=1e6, mutable=True)
    
    model.exchbal_violation_pos = Var(model.timesteps, domain=NonNegativeReals)
    model.exchbal_violation_neg = Var(model.timesteps, domain=NonNegativeReals)
    
    def global_exchange_balance_soft_rule(m, t):
        if t == t0:
            return Constraint.Skip
        return (
            sum(m.exchange[n, t] for n in m.nodes)
            == m.exchbal_violation_pos[t] - m.exchbal_violation_neg[t]
        )
    
    model.global_exchange_balance = Constraint(model.timesteps, rule=global_exchange_balance_soft_rule)
    ...
    model.obj = Objective(
        expr=(
            ...
            + sum(
                model.PEN_PHS_PROFILE * (model.phs_profile_under[ca, t] + model.phs_profile_over[ca, t])
                for ca in model.control_areas
                for t in model.timesteps
                if t != t0
            )
            + sum(
                (model.PEN_PTDFFLOW * (model.ptdf_violation_pos[c, t] + model.ptdf_violation_neg[c, t]))
                for c in model.connections for t in model.timesteps if t != t0
            )
            + sum(
                (model.PEN_FLOWCAP * (model.flow_over_pos[c, t] + model.flow_over_neg[c, t]))
                for c in model.connections for t in model.timesteps if t != t0
            )
            + sum(
                (model.PEN_EXCHBAL * (model.exchbal_violation_pos[t] + model.exchbal_violation_neg[t]))
                for t in model.timesteps if t != t0
            )
                ),
        sense=minimize,
    )
...
def solve_model(
    ...
    tee: bool = False,
    tag: str = "",
    iis_timelimit_s: int = 300,
    write_lp: bool = False,     # NEW
    write_iis: bool = True,     # NEW (keep IIS optional)
):

    ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    tag_part = f"_{tag}" if tag else ""

    lp_path  = os.path.abspath(f"model_debug{tag_part}_{ts}.lp")
    ilp_path = os.path.abspath(f"model_debug{tag_part}_{ts}.ilp")

    solver = SolverFactory("gurobi_persistent")
    solver.set_instance(model)

    # Write LP with symbolic names
    if write_lp:
        model.write(lp_path, io_options={"symbolic_solver_labels": True})
        print(f"📝 Wrote LP: {lp_path}")
    ...
    ok = (st == SolverStatus.ok) and (tc in {
    ...
    if tc in {TerminationCondition.infeasible,
        ...
        g = solver._solver_model  # gurobipy.Model

        print("\n❌ Model infeasible — computing IIS...")

        # Make IIS more robust & bounded
        g.Params.DualReductions = 0
        g.Params.NumericFocus = 3
        g.Params.IISMethod = 1
        g.Params.TimeLimit = iis_timelimit_s

        g.computeIIS()
        if write_iis:
            g.write(ilp_path)
            print(f"🧾 Wrote IIS: {ilp_path}")

        # Fallback: print Gurobi-side constraint names
        iis_rows = [c for c in g.getConstrs() if c.IISConstr]
        print(f"\nIIS constraints (Gurobi-side): {len(iis_rows)}")
        for c in iis_rows[:200]:
            print(" -", c.ConstrName)

        msg = f"Infeasible model.\nstatus={st}, termination={tc}"
        if write_lp:
            msg += f"\nLP:  {lp_path}"
        if write_iis:
            msg += f"\nIIS: {ilp_path}"
    ...
    raise RuntimeError(
        f"Solve failed: status={st}, termination={tc}"
    )